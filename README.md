...
- Run `npm run dev` for local development. It starts both the Vite frontend and the Express payment server.
- In production, deploy the Node server from `server/index.js` separately and set `VITE_API_BASE_URL` in the frontend to your deployed backend URL.
- Add `RAZORPAY_KEY_ID` and `RAZORPAY_KEY_SECRET` to the backend environment before starting the server.
